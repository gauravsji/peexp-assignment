		...
		<div class="wrapper">
			...
			<div class="content-wrapper">
				<section class="content-header">
					<h1>
						Order Details
						<a href=<?php echo $url."/reports/order_report_html.php"; ?> class="btn pull-right">
					...
					<div class="box">
						<div class="box-body pad">
							<div class="row">
								<div class="col-xs-12">
									...
									<i></i>  <center><h3>Order For:  <strong><?php echo $order_result['order_brief_details']; ?></strong></h3></center>
										...
										<br><br>
											<?php
											$sql = "SELECT * FROM `ss_order` WHERE order_id=".$order_id;
											$result=mysqli_query($conn,$sql);
											$payment_result = mysqli_fetch_array($result,MYSQLI_ASSOC);
											if($payment_result['order_status']=="Material Delivered")
											{
												if($payment_result['payment_status']=="Uninvoiced")
												{
													echo"<div class='pull-left'><a class='btn btn-info btn-sm' disabled>".$payment_result['payment_status']."</a></div>";
												}
												else
												{
													if($payment_result['send_pdf_status']==1)
													{
														echo"<div class='pull-left'><a target='_blank' href='../../php/order_pdf.php?id=" . $order_id. "'class='btn btn-info btn-sm' onClick='invoice_clicked()'>View Invoice</a></div>";
													}
													else if($payment_result['send_image_status']==1)
													{
														$sql = "SELECT * FROM `send_invoice_image` WHERE delete_status=0 and module_name='order' and module_id=".$order_id;
														$result = mysqli_query($conn,$sql);
														while ($row = mysqli_fetch_array($result))
														{
														  echo  "<div class='pull-left'><a target='_blank' href=".$row['photo_name']. "' class='btn btn-info btn-xs' onClick='invoice_clicked()'>View Invoice</a></div>";
														}
													}

												}
											}
											if($payment_result['payment_status']=="Processed"|| $payment_result['payment_status']=="Unpaid")
												{

													echo '<button type="button" class="btn btn-primary btn-flat btn-sm pull-right" data-toggle="modal"  data-target="#Raise_an_Issue">Raise an Issue Image</button>';
													
												}

										?>
										<script>
										function invoice_clicked()
										{
											console.log("asd");
											var order_id= <?php echo $order_result['order_id'];?>;
											$.ajax(
											{
												url: "../../php/update/update_payment_status.php",
												type: "POST", // you can use GET
												data: {order_id:order_id}, // post data
							...
							<table class="table table-bordered table-condensed table-sm table-responsive" id="view_order_photo_html"cellspacing="0" width="100%">
								...
								<tbody>
								<?php
								$sql = "SELECT * FROM `proof_of_delivery_photo` WHERE delete_status=0 and module_name='order' and module_id=".$order_id;
									$result = mysqli_query($conn,$sql);
									while ($row = mysqli_fetch_array($result))
									{
									// Print out the contents of the entry
									echo '<tr><td><center><img width="20%" class="fancybox" height="20%" src="'.$row['photo_name'].'"/></center></td>';
									}
								?>
								</tbody>
							</table>
...
<div id="Raise_an_Issue" class="modal fade" role="dialog">
		<div class="modal-dialog  modal-lg">
			...
			<div class="modal-content">
				<div class="modal-header">
					<button type="button"  class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title" align="center"> Want to raise an Issue ?</h4>
				</div>
				<div class="modal-body">
					<div class="table-responsive">
								<div class="form-group col-md-12">
								<h4 class="modal-title" align="Left"> Add Remark :</h4>
									<div class="input-group">
										<textarea rows="12" cols="146" id="ui_payment_status"></textarea>
									</div>
								</div>
								<div class="col-lg-offset-10 col-lg-2">
									<button type="submit"  align="center"  data-loading-text="Please Wait..." onClick='submit_clicked()' class="btn btn-success form-control">Submit</button>
								</div>							
							</div>
							<script>
									...
									var order_id= <?php echo $order_result['order_id'];?>;
											...
											var remark= $("#ui_payment_status").val();
										console.log(remark);

											$.ajax(
											{

												url: "../../php/update/update_raise_an_issue.php",
												type: "POST", // you can use GET
												data: {order_id:order_id, remark:remark}, // post data
												success: function(data)   // A function to be called if request succeeds
												{
