...
<html>
	...
	$sql = "SELECT * FROM enquiry where enquiry_id = " . $ss_enquiry_id;
	$result = mysqli_query($conn, $sql);
	$enquiry_result = mysqli_fetch_array($result,MYSQLI_ASSOC);
		
	$sql4 = "SELECT * FROM enquiry_product where delete_status<>1 and enquiry_id = " . $_GET["id"];
				...
				<section class="content-header">
					<h1>Edit Enquiry  
						<div class="btn-toolbar pull-right">		
							<a href="../html/view_enquiry_html.php?id=<?php echo $ss_enquiry_id;?>" class="btn btn-sm btn-info">View Enquiry</a>  						
							<a href="../html/add_enquiry_html.php" class="btn btn-sm btn-primary">New Enquiry</a>  
							<a href="../reports/enquiry_report_html.php" class="btn btn-sm btn-success">Enquiry Report</a>							
					...
					<div class="row">
						...
						<div class="col-md-12">
							...
							<div class="box box-primary">
								...
								<form action="../php/update/update_enquiry_php.php" method="POST" onsubmit="submit.disabled = true; return true;">
								...
								<div class="form-group col-md-3">
									...
									<div class="input-group">
										<span class="input-group-addon"><i class="fa fa-user"></i></span>
										<select name="ui_sales_lead" id="ui_sales_lead" class='form-control select2' style='width: 100%;'>
										<option selected disabled hidden value="0">Select Lead</option>
										<?php
										{
											...
											while($row = mysqli_fetch_array($query))
											{
												if ($row['sales_lead_id'] == $enquiry_result['sales_lead_id']):
												{
												echo "<option value='" . $row['sales_lead_id'] . "' selected>" . $row['sales_lead_name']. "</option>";
												}
												else:
												{
													echo "<option value='" . $row['sales_lead_id'] . "'>" . $row['sales_lead_name']. "</option>";
												}
												endif;
											}
										} 
								...
								<div class="form-group col-md-3">
									...
									<div class="input-group">
										...
										<?php
										{
											$sql = "SELECT * from enquiry where delete_status<>1";
											$query = mysqli_query($conn, $sql);
											while($row = mysqli_fetch_array($query))
											{
												if ($row['enquiry_id'] == $enquiry_result['enquiry_id']):
												{
													if ($enquiry_result['enquiry_status']=='OPEN - QUOTE TO BE SENT')
													{
														echo '<option selected>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														echo '<option>OPEN - PRODUCT RESEARCH PENDING</option>';
														echo '<option>OPEN - QUOTE SENT, AWAITING APPROVAL</option>';
														echo '<option>CLOSED - REJECTED, PRICE TOO HIGH</option>';
														echo '<option>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
														echo '<option>CLOSED - REJECTED, DELAYED REPONSE</option>';
														echo '<option>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
														echo '<option>CLOSED - VENDOR NOT FOUND</option>';
														echo '<option>CLOSED - CONVERTED TO ORDER</option>';
													}
													else if ($enquiry_result['enquiry_status']=='OPEN - AWAITING PRODUCT INFO FROM CLIENT')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option selected>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														echo '<option>OPEN - PRODUCT RESEARCH PENDING</option>';
														echo '<option>OPEN - QUOTE SENT, AWAITING APPROVAL</option>';
														echo '<option>CLOSED - REJECTED, PRICE TOO HIGH</option>';
													...
													else if ($enquiry_result['enquiry_status']=='OPEN - AWAITING PRODUCT INFO FROM VENDOR')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option selected>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														echo '<option>OPEN - PRODUCT RESEARCH PENDING</option>';
														echo '<option>OPEN - QUOTE SENT, AWAITING APPROVAL</option>';
														echo '<option>CLOSED - REJECTED, PRICE TOO HIGH</option>';
														echo '<option>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
													...
													else if ($enquiry_result['enquiry_status']=='OPEN - PRODUCT RESEARCH PENDING')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														echo '<option selected>OPEN - PRODUCT RESEARCH PENDING</option>';
														echo '<option>OPEN - QUOTE SENT, AWAITING APPROVAL</option>';
														echo '<option>CLOSED - REJECTED, PRICE TOO HIGH</option>';
														echo '<option>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
														echo '<option>CLOSED - REJECTED, DELAYED REPONSE</option>';
													...
													else if ($enquiry_result['enquiry_status']=='OPEN - QUOTE SENT, AWAITING APPROVAL')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														echo '<option>OPEN - PRODUCT RESEARCH PENDING</option>';
														echo '<option selected>OPEN - QUOTE SENT, AWAITING APPROVAL</option>';
														echo '<option>CLOSED - REJECTED, PRICE TOO HIGH</option>';
														echo '<option>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
														echo '<option>CLOSED - REJECTED, DELAYED REPONSE</option>';
														echo '<option>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
													...
													else if ($enquiry_result['enquiry_status']=='CLOSED - REJECTED, PRICE TOO HIGH')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														echo '<option>OPEN - PRODUCT RESEARCH PENDING</option>';
														echo '<option>OPEN - QUOTE SENT, AWAITING APPROVAL</option>';
														echo '<option selected>CLOSED - REJECTED, PRICE TOO HIGH</option>';
														echo '<option>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
														echo '<option>CLOSED - REJECTED, DELAYED REPONSE</option>';
														echo '<option>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
														echo '<option>CLOSED - VENDOR NOT FOUND</option>';
													...
													else if ($enquiry_result['enquiry_status']=='CLOSED - REJECTED, NOT THE RIGHT PRODUCT')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														...
														echo '<option>OPEN - QUOTE SENT, AWAITING APPROVAL</option>';
														echo '<option>CLOSED - REJECTED, PRICE TOO HIGH</option>';
														echo '<option selected>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
														echo '<option>CLOSED - REJECTED, DELAYED REPONSE</option>';
														echo '<option>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
														echo '<option>CLOSED - VENDOR NOT FOUND</option>';
														echo '<option>CLOSED - CONVERTED TO ORDER</option>';
													}
													else if ($enquiry_result['enquiry_status']=='CLOSED - REJECTED, DELAYED REPONSE')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														...
														echo '<option>CLOSED - REJECTED, PRICE TOO HIGH</option>';
														echo '<option>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
														echo '<option selected>CLOSED - REJECTED, DELAYED REPONSE</option>';
														echo '<option>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
														echo '<option>CLOSED - VENDOR NOT FOUND</option>';
														echo '<option>CLOSED - CONVERTED TO ORDER</option>';
													}
													else if ($enquiry_result['enquiry_status']=='CLOSED - CLIENT CHANGED REQUIREMENT')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														...
														echo '<option>CLOSED - REJECTED, NOT THE RIGHT PRODUCT</option>';
														echo '<option>CLOSED - REJECTED, DELAYED REPONSE</option>';
														echo '<option selected>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
														echo '<option>CLOSED - VENDOR NOT FOUND</option>';
														echo '<option>CLOSED - CONVERTED TO ORDER</option>';
													}
													else if ($enquiry_result['enquiry_status']=='CLOSED - VENDOR NOT FOUND')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														...
														echo '<option>CLOSED - REJECTED, DELAYED REPONSE</option>';
														echo '<option>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
														echo '<option selected>CLOSED - VENDOR NOT FOUND</option>';
														echo '<option>CLOSED - CONVERTED TO ORDER</option>';
													}
													else if ($enquiry_result['enquiry_status']=='CLOSED - CONVERTED TO ORDER')
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM CLIENT</option>';
														echo '<option>OPEN - AWAITING PRODUCT INFO FROM VENDOR</option>';
														...
														echo '<option>CLOSED - CLIENT CHANGED REQUIREMENT</option>';
														echo '<option>CLOSED - VENDOR NOT FOUND</option>';
														echo '<option selected>CLOSED - CONVERTED TO ORDER</option>';
													}
													else 
													{
														echo '<option>OPEN - QUOTE TO BE SENT</option>';
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form method="post" id="export_excel">  
					 ...
					 <a href="../php/test/upload.xlsx" download>Download Format </a>
					 <br/>
...
<div class="modal fade" id="edit_enquiry_product_modal" role="dialog">
		...
		$("#save_line_item_submit").click(function() {
			...
			var draft_id=<?php echo $ss_enquiry_id; ?>;				
