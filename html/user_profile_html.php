...
<div class="tab-pane" id="users">
	<section class='invoice'>
						...
						<table id="view_brand_html" class="table table-bordered">
							...
							<tbody>
								...
								while ($row = mysqli_fetch_array($result))
								{
									...
									echo  "<td><center> <button name='reset_password".$row['id']."' data-id='".$row['id']."' value='".$row['email']."'
									id='reset_password' class='btn btn-sm btn-primary reset_password".$row['id']."'>Reset</button></center></td></tr>";
								}
								?>
							</tbody>
						</table>
		...
		<script type="text/javascript">
			...
			$("button[name^='reset_password']").click(function() {			
				var user_id = $(this).attr('data-id');
				var user_email = $(this).attr('value');

			$.ajax(
			{    
				url: "../php/live_update/update_user_password_admin.php",
				type: "POST", // you can use GET
				data: {user_id: user_id,user_email:user_email}, // post data
