...
if($type == "Rfq1")
{
	...
	(order_id, order_product_name, order_product_description, order_product_quantity, order_buying_price, order_discounted_price, order_discount_percent, order_total_of_buying, order_selling_percentage, order_selling_price, order_tax, tax_inclusive, order_total,rfq_id)
	(SELECT '".$order_draft_id."' as order_id, product_name , product_description, product_quantity
	...
	, enquiry_total ,product_enquiry_id FROM customer_rfq_enquiry where delete_status<>1 and product_status = 'Accept' and id in ($product_list));";
}
else {
	$sql1 = "insert into order_product
	(order_id, order_product_name, order_product_description, order_product_quantity, order_buying_price, order_discounted_price, order_discount_percent, order_total_of_buying, order_selling_percentage, order_selling_price, order_tax, tax_inclusive, order_total,rfq_id)
	(SELECT '".$order_draft_id."' as order_id, product_name , product_description, product_quantity
	, enquiry_buying_price, enquiry_discounted_price, enquiry_discount_percent, enquiry_total_of_buying, enquiry_selling_percentage, enquiry_selling_price, enquiry_tax,tax_inclusive
	, enquiry_total ,product_enquiry_id FROM customer_rfq_enquiry where delete_status<>1 and product_status = 'Accept' and product_enquiry_id=$id);";
}
$result1 = mysqli_query($conn, $sql1);

$order_result = mysqli_fetch_array($result1,MYSQLI_ASSOC);
$sql2 = "SELECT * FROM rfq where rfq_id = " . $id;
...
<head>
	...
	$(document).ready(function()
	{
		...
		fetchMysqlDatabase("<?php if($type=="Rfq")
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="box box-primary">
                            ...
                            <!-- /.box-header -->
                            <div class="box-body">
                                <form name="add_order_form" id="add_order_form" enctype="multipart/form-data" action="../php/add/add_rfq_order_php.php" method="post">
								...
								<input type="hidden" name="ui_order_placed_by" id="ui_order_placed_by" value ="<?php echo $id; ?>"/>
								<!--Order ID-->
...
else
{
	...
	$sql = "SELECT * from vendor where delete_status<>1";
								...
								<div class="form-group col-md-3">
									...
									<div class="input-group">
										...
										<input type="hidden" id="ui_order_placed_by" name="ui_order_placed_by" value=<?php echo $enquiry_result['data_entered_by'];  ?>>
                    <?php
                        $customer_query = "SELECT * FROM customer WHERE customer_id=".$enquiry_result['data_entered_by'];
                        $customer_result = mysqli_query($conn, $customer_query);
                        $result = mysqli_fetch_array($customer_result,MYSQLI_ASSOC);
                        echo '<input type="text" class="form-control" maxlength="25" class="form-control" id="ui_order_placed_by_show" name="ui_order_placed_by_show" value="'.$result['customer_name'].'">';
                     ?>
									</div>
								</div>
								<!--Order Placed By-->
								...
								<div class="form-group col-md-3">
									...
									<div class="input-group">
										...
										<?php
											echo '<option value="customer_panel" selected>Customer Panel</option>';
										?>
										</select>
									</div>
								</div>
                        ...
                        echo '<option value="Within State" selected>Within State</option>';
								...
								<div class="form-group col-md-6">
									...
									<textarea class="form-control" rows="4" id="ui_order_remarks" name="ui_order_remarks" maxlength="150" ></textarea>
								</div>
								<!--Order Remarks-->
								<!--File Upload
								<div class="form-group col-md-3">
    ...
    <!--Including Bootstrap and other scripts-->
		...
		$("#save_line_item_submit").click(function() {
			...
			var draft_id= <?php echo $order_draft_id; ?>;
			...
			var product_discount_percent= $("#modal_product_discount_percent").val();
			var product_discount_price = $("#modal_product_discounted_price").val();
			...
			$.ajax(
			{
				url: "../php/add_modal/add_rfq_order_php.php",
				type: "POST", // you can use GET
				data: {draft_id: draft_id, product_discount_price:product_discount_price,product_name: product_name, product_description: product_description, product_quantity:product_quantity,product_buying_price:product_buying_price,discounted_price:discounted_price,total_of_buying:total_of_buying,product_selling_percent:product_selling_percent,product_selling_price:product_selling_price,product_tax:product_tax,product_tax_inclusive:product_tax_inclusive,product_total:product_total,user_id:user_id,location:location}, // post data
  ...
  function handleChange(input) {
    ...
    if (input.value > 100) input.value = 100;
  	...
  	$("#save_edit_line_item_submit").click(function() {
			...
			var product_discount_percent= $("#edit_modal_product_discount_percent").val();
			...
			$.ajax(
			{
				url: "../php/update/update_order_rfq_php.php",
				type: "POST", // you can use GET
				data: {product_discount_percent:product_discount_percent,order_id:order_id,order_product_id: order_product_id, product_name: product_name, product_description: product_description, product_quantity: product_quantity,product_buying_price: product_buying_price, discounted_price: discounted_price, total_of_buying: total_of_buying,product_selling_percent: product_selling_percent, product_selling_price: product_selling_price, product_tax: product_tax, product_tax_inclusive: product_tax_inclusive, product_total: product_total, user_id: user_id,location: location}, // post data
				success: function(data)   // A function to be called if request succeeds
				{
					$("#myModal .close").click()
