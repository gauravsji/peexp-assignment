...
foreach ($list as $rs) {
	...
	$product_name = str_replace($_POST['keyword'], '<b>'.$_POST['keyword'].'</b>', $rs['auto_complete_product_name']);
	// add new option
    echo '<li onclick="edit_set_item(\''.str_replace("'", "\'", $rs['auto_complete_product_name']).'\','.$rs['product_id'].')">'.$product_name.'</li>';
}
?>