...
function addClientAdresse( $adresse )
{
	...
	$this->SetXY( $r1, $y1);
	$this->SetFont('Arial','B',14);
	$this->Cell($r1,4, "Billing Address");
	$this->SetXY( $r1, $y1 + 6 );
	$this->SetFont('Arial','',10);
...
// Shipping address
function addshippingAdresse( $adresse )
{
	$r1     = $this->w-70;
	$r2     = $r1;
	$y1     = 50;
	$this->SetXY( $r1, $y1);
	$this->SetFont('Arial','B',14);
	$this->Cell($r1,4, "Shipping Address");
	$this->SetXY( $r1, $y1 + 6 );
	$this->SetFont('Arial','',10);
	$this->MultiCell( 60, 4, $adresse);
}
...
function addtotalbox($tax_amount)
{
	$r1  = $this->w - 200;
	$r2  = $r1 + 190;
	$y1  = $this->h - 50;
	$y2  = $y1+35;
	$this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2-$y1), 0.1, 'D');
	$this->SetFont( "Arial", "B", 8);
	$this->SetXY( $r1+140, $y1+3 );
	$this->Cell(20,4, "Total:", 0, 0, "L");
	$this->SetXY( $r1+140, $y1+8 );
	$this->Cell(20,4, "Tax:".$tax_amount, 0, 0, "L");
	$this->SetXY( $r1+140, $y1+13 );
	$this->Cell(20,4, "Transport:", 0, 0, "L"); 
	$this->SetXY( $r1+140, $y1+22 );
	$this->Cell(20,4, "Grand Total:", 0, 0, "L"); 
}
