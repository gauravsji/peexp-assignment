<?php
	...
	$mailbody.="<tr bgcolor='#ffb732' width='100%'><td bgcolor='#ffb732'><table><tr>
			...
			<td align='right' width='60%'><h3> ESTIMATE: ".$enquiry_result['rfq_id']."</h3></td>
			</tr></table></td>
			</tr>";
	...
	$query_result1 = 'SELECT * FROM rfq e,customer_rfq_enquiry ep where e.rfq_id='.$enquiry_id.' and e.rfq_id=ep.product_enquiry_id and ep.delete_status<>1';
...
<tr style='border: 1px solid #cccccc; padding: 10px 10px 10px 10px;'><th align='center' style='border: 1px solid #cccccc;  padding: 6px 6px 6px 6px;'>Product Name</th><th align='center' style='border: 1px solid #cccccc;  padding: 6px 6px 6px 6px;'>Product Description</th><th align='center' style='border: 1px solid #cccccc;  padding: 6px 6px 6px 6px;'>Quantity</th><th align='center' style='border: 1px solid #cccccc;  padding: 6px 6px 6px 6px;'>Price</th><th align='center' style='border: 1px solid #cccccc;  padding: 6px 6px 6px 6px;'>Tax</th><th align='center' style='border: 1px solid #cccccc;  padding: 6px 6px 6px 6px;'>Tax I/E</th><th align='center' style='border: 1px solid #cccccc;  padding: 6px 6px 6px 6px;'>Total</th></tr>";
	...
	{
		$mailbody.="<tr style='border: 1px solid #cccccc;'><td width='25%' align ='center' style='border: 1px solid #cccccc;'>".$row['product_name']."</td>";
		$mailbody.="<td width='20%' align ='center' style='border: 1px solid #cccccc;'>".$row['product_description']."</td>";
		$mailbody.="<td width='20%' align ='center' style='border: 1px solid #cccccc;'>".$row['product_quantity']."</td>";
	...
	{ 	*/
		$PI_status = '';
		if($enquiry_result['role'] == 'admin' || $enquiry_result['role'] == 'Admin'){
			$PI_status ='approved';
		}
		else{
			$PI_status ='approved';
		...
		$pi_number = mt_rand(1000, 9999);
		$PI_date =date('Y-m-d');
		$status = 'Quote Sent';
		$add_estimate_to_enquiry_query ="Update rfq SET enquiry_estimate_message = '$email_body',
			PI_number ='$pi_number',
			PI_Date = '$PI_date',
			pi_status ='$PI_status',
