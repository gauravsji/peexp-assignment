						...
						<div class="box-body">
							...
							<table id="view_task_html" class="table table-bordered dt-responsive table-condensed table-striped">
								...
								<tbody>
									...
									{
									$sql = "SELECT * FROM task t, users u where t.task_assignee=u.id and t.delete_status<>1 and t.location='".$user_result['location']."' and t.task_assignee='".$user_result['id']."' order by t.task_status";
									}
									else
									{
										$sql = "SELECT * FROM task t, users u where t.task_assignee=u.id and t.delete_status<>1 and t.task_assignee='".$user_result['id']."' order by t.task_status";
									}
									...
									echo '<td align="center" style="width:10%" contenteditable="true" onBlur=\'saveToDatabase(this,"task_start_date",'. $row['task_id'].')\' onClick="showEdit(this)">'. date("d-m-Y", strtotime($row['task_start_date'])) .'</td>';
									
									echo '<td align="center" style="width:10%"  contenteditable="true" onBlur=\'saveToDatabase(this,"task_due_date",'. $row['task_id'].')\' onClick="showEdit(this)">'. date("d-m-Y", strtotime($row['task_due_date'])).'</td>';
