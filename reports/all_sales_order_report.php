...
<table id="view_order_html" class="sieve table table-bordered table-striped table-fixed table-condensed" style="border-collapse:collapse;">
		...
		{
			$sql = "SELECT * FROM ss_order o,vendor v,customer c,project p where  o.customer_id=c.customer_id and o.project_id=p.project_id and o.delete_status=0 and order_date > DATE_SUB( CURDATE( ) , INTERVAL 1 YEAR ) and o.vendor_id != 433 order by o.order_id desc";
		}
		else
		{
			$sql = "SELECT * FROM ss_order o,vendor v,customer c,project p where o.vendor_id != 433 and o.customer_id=c.customer_id and o.project_id=p.project_id and o.delete_status=0 order by o.order_id desc";
		}
