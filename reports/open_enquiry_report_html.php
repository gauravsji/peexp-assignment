								...
								<tbody>
									<?php
										...
										$sql = "SELECT * FROM enquiry e
												...
												WHERE e.delete_status <> 1 and e.location='".$user_result['location']."' and (e.enquiry_status='OPEN - QUOTE TO BE SENT' or e.enquiry_status='OPEN - AWAITING PRODUCT INFO FROM CLIENT' or e.enquiry_status='OPEN - AWAITING PRODUCT INFO FROM VENDOR' or e.enquiry_status='OPEN - PRODUCT RESEARCH PENDING' or e.enquiry_status='OPEN - QUOTE SENT, AWAITING APPROVAL')";
										}
										else
										{
											$sql = "SELECT * FROM enquiry e
												...
												LEFT OUTER JOIN project p ON e.project_id = p.project_id
												LEFT OUTER JOIN users u ON u.id = e.enquiry_assignee
												WHERE e.delete_status <> 1 and e.enquiry_status='OPEN - QUOTE TO BE SENT' or e.enquiry_status='OPEN - AWAITING PRODUCT INFO FROM CLIENT' or e.enquiry_status='OPEN - AWAITING PRODUCT INFO FROM VENDOR' or e.enquiry_status='OPEN - PRODUCT RESEARCH PENDING' or e.enquiry_status='OPEN - QUOTE SENT, AWAITING APPROVAL'";
										}
										...
										while ($row = mysqli_fetch_array($result))
										{
											...
											{
												echo '<td style="width:12%"><div class="badge bg-blue">' . $row['enquiry_status'] . '</div></td>';
											}
											...
											{
											echo '<td style="width:12%"><div class="badge bg-green"><center>' . $row['enquiry_status'] . '</div></td>';
											}															
