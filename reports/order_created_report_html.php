...
<table id="view_order_html" class="sieve table table-bordered table-striped table-fixed" style="border-collapse:collapse;">
	...
	<tbody>
		...
		{
			$sql = "SELECT * FROM ss_order o,vendor v,customer c,project p where o.vendor_id=v.vendor_id and o.customer_id=c.customer_id and o.project_id=p.project_id and o.delete_status='0' and order_status='Order Created' and o.location='".$user_result['location']."'";
		}
		else
		{
			$sql = "SELECT * FROM ss_order o,vendor v,customer c,project p where o.vendor_id=v.vendor_id and o.customer_id=c.customer_id and o.project_id=p.project_id and o.delete_status='0' and order_status='Order Created'";
		}
			...
			while ($row = mysqli_fetch_array($result))
			{
				...
				echo '<td><center>' . $row['order_id'] . '</center></td><td><center>' . date("d-m-Y", strtotime($row['order_date'])) . '</center></td><td><center>' . $row['order_brief_details'] . '</center></td><td><center>' . $row['vendor_name'] . '</center></td><td><center>' . $row['customer_name'] . '</center></td><td><center>' . $row['project_name'] . '</center></td>';
				...
				echo  "<td><center> <a href='../html/edit_order_html.php?id=" . $row['order_id'] . "' class='btn btn-warning btn-xs'>Edit</a></center></td>";
				if($user_result['role']=="Admin")
				{
