...
<table id="view_order_html" class="sieve table table-bordered table-striped table-fixed table-condensed" style="border-collapse:collapse;">
		...
		<thead>
			...
			<th><center>Tally No</center></th>			
			<th><center>PB No</center></th>			
			<th><center>Products</center></th>
		...
		else
		{
			...
			{
				/* Total amount */
				$sql_total = "SELECT sum(order_total) as order_total_sum FROM ss_order o,order_product p where o.order_id = p.order_id and o.order_id = ".$row['order_id'];
				$result_total = mysqli_query($conn,$sql_total);
				$order_total = mysqli_fetch_array($result_total,MYSQLI_ASSOC);
				/* Transport amount */
				$sql_trans = "SELECT order_transportation_charge FROM ss_order o,order_transport p where o.order_id = p.order_id and o.order_id = ".$row['order_id'];
				$result_trans = mysqli_query($conn,$sql_trans);
				$order_trans = mysqli_fetch_array($result_trans,MYSQLI_ASSOC);
				
				$total_amount =  round( $order_total['order_total_sum']+$order_trans['order_transportation_charge']);
				...
				echo '<td><center><a href="../html/view_order_html.php?id=' . $row['order_id'] . '">' . $row['order_id'] . '</a></center></td>';
				...
				$tally_info = $order_account_result['order_ss_invoice_number'];
				$purchase_info = $order_account_result['order_purchase_bill_number'];
					}
				echo '<td><center>' . $tally_info. '</center></td>';	//7
				echo '<td><center>' . $purchase_info. '</center></td>';
